...
constexpr uint32_t kSensorLedUpdateIntervalMs = 100;
constexpr uint32_t kLedBlinkIntervalMs = 250;
constexpr uint32_t kHidReportIntervalMs = 15;
constexpr uint32_t kDiagnosticPrintIntervalMs = 2000;
constexpr uint32_t kDebounceMs = 30;
constexpr uint32_t kDoubleClickWindowMs = 450;
...
Chain M5Chain;
BleGamepad bleGamepad("Boat Angle Controller", "M5Stack", 100);
Preferences prefs;
...
void readSensors() {
  for (uint8_t i = 0; i < sensorCount; ++i) {
    ...
    if (M5Chain.getAngle12BitAdc(sensorIds[i], &value) == CHAIN_OK) {
      sensorRaw[i] = value;
    }
  }
}
...
void sendGamepadReport() {
  ...
  int16_t x = sensorCount > 0 ? rawToAxis(0, sensorRaw[0]) : 0;
  int16_t y = sensorCount > 1 ? rawToAxis(1, sensorRaw[1]) : 0;
  int16_t rx = sensorCount > 2 ? rawToAxis(2, sensorRaw[2]) : 0;
  int16_t ry = sensorCount > 3 ? rawToAxis(3, sensorRaw[3]) : 0;

  bleGamepad.setAxes(x, y, 0, rx, ry, 0, 0, 0);
...
void updateLeds() {
  ...
  if (mode == Mode::AngleCalibration) {
    setAtomLed(blinkOn ? Rgb{0, 80, 0} : Rgb{0, 0, 0});
    return;
  }

  if (!bleGamepad.isConnected()) {
    setAtomLed(blinkOn ? Rgb{80, 0, 0} : Rgb{0, 0, 0});
    return;
  }
...
void setup() {
  ...
  config.setAxesMin(kAxisMin);
  config.setAxesMax(kAxisMax);
...
void loop() {
  ...
  if ((now - lastDiagnosticPrintMs) >= kDiagnosticPrintIntervalMs) {
    lastDiagnosticPrintMs = now;
    Serial.printf("Diag: mode=%u button=%u angles=%u ble=%u\r\n", static_cast<unsigned>(mode),
                  digitalRead(kAtomButtonPin) == LOW, sensorCount, bleGamepad.isConnected());
  }
  ...
  if ((now - lastSensorLedUpdateMs) >= kSensorLedUpdateIntervalMs) {
    lastSensorLedUpdateMs = now;
    updateSensorStatusLeds();
  }

  if (mode == Mode::Normal && (now - lastHidReportMs) >= kHidReportIntervalMs) {
    lastHidReportMs = now;
    sendGamepadReport();
  }
